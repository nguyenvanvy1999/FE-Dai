...
function Support() {
  ...
  return (
    <MainLayout>
      <form className="container mx-auto px-2 md:px-14" onSubmit={handleSubmit(onSubmit)}>
        <div className="grid gap-2 md:gap-10 mb-6 grid-cols-1 md:grid-cols-2 p-5 md:p-10 border border-gray-300 rounded-lg">
          <div>
            ...
            <div className="mt-10">
              ...
              <ul className="mt-5">
                {ContactList.map((contact, idx) => (
                  <li key={contact.title} className={`px-1 ${idx <= 1 && 'text-green-600'}`}>
                    <Link
                      to="/"
                      className={`mt-3 flex items-center gap-2 text-sm text-black-light font-normal ${
              ...
              <p>
                Nhân viên hỗ trợ của chúng tôi sẽ cố gắng xử lý khiếu nại và giải quyết thắc mắc của
                các bạn nhanh nhất có thể.
              </p>
            </div>
          </div>
          <div>
            <p className="text-left font-semibold text-xl mt-10 md:mt-2">Tin nhắn</p>
            <div className="mt-2">
